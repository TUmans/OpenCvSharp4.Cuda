...
public class VectorOfGpuMat : CvObject, IStdVector<GpuMat>
{
    ...
    {
        var p = NativeMethods_cuda.vector_GpuMat_new1();
        SetSafeHandle(new OpenCvCudaPtrSafeHandle(p, ownsHandle: false, releaseAction: null));
    }
    ...
    public VectorOfGpuMat(int size)
    {
        ...
        var p = NativeMethods_cuda.vector_GpuMat_new2((uint)size);
        SetSafeHandle(new OpenCvCudaPtrSafeHandle(p, ownsHandle: false, releaseAction: null));
    }
    ...
    public VectorOfGpuMat(IEnumerable<GpuMat> mats)
    {
        ...
        var p = NativeMethods_cuda.vector_GpuMat_new3(
            matPointers,
    ...
    {
        NativeMethods_cuda.vector_GpuMat_delete(CvPtr);
    ...
    public int Size
    {
        get
        {
            var res = NativeMethods_cuda.vector_GpuMat_getSize(CvPtr);
            GC.KeepAlive(this);
            return (int)res;
        }
    }
    ...
    public IntPtr ElemPtr
    {
        get
        {
            var res = NativeMethods_cuda.vector_GpuMat_getPointer(CvPtr);
            GC.KeepAlive(this);
            return res;
        }
    }
        ...
        where T : GpuMat, new()
    {
        ...
        NativeMethods_cuda.vector_GpuMat_assignToArray(CvPtr, dstPtr);
        GC.KeepAlive(this);
