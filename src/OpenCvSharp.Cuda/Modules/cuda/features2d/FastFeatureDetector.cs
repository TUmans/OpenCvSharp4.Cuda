...
public class FastFeatureDetector : Feature2DAsync
{
    ...
    private FastFeatureDetector(IntPtr smartPtr, IntPtr rawPtr)
        : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_FastFeatureDetector_delete(p)))
    {
    }
        ...
        int maxNPoints = 5000)
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createFastFeatureDetector(
                threshold, nonmaxSuppression ? 1 : 0, (int)type, maxNPoints, out var smartPtr));

        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_FastFeatureDetector_get(smartPtr, out var rawPtr));
    ...
    public int MaxNumPoints
    {
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_FastFeatureDetector_getMaxNumPoints(RawPtr, out int val));
            GC.KeepAlive(this);
            return val;
        }
        set
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_FastFeatureDetector_setMaxNumPoints(RawPtr, value));
            GC.KeepAlive(this);
        }
    }
    ...
    public int Threshold
    {
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_FastFeatureDetector_setThreshold(RawPtr, value));
            GC.KeepAlive(this);
        }
    }
}
