...
public class CannyEdgeDetector : Algorithm
{
    ...
    {
        NativeMethods.HandleException(NativeMethods_cuda.cuda_createCannyEdgeDetector(
            lowThresh, highThresh, appertureSize, l2Gradient ? 1 : 0, out var smartPtr));

        NativeMethods.HandleException(NativeMethods_cuda.cuda_CannyEdgeDetector_get(smartPtr, out IntPtr rawPtr));
    ...
    public virtual void Detect(OpenCvSharp.Cuda.InputArray image, OpenCvSharp.Cuda.OutputArray edges, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CannyEdgeDetector_detect(RawPtr, image.CvPtr, edges.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
        ...
        OpenCvSharp.Cuda.OutputArray edges, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CannyEdgeDetector_detect_dxdy(RawPtr, dx.CvPtr, dy.CvPtr, edges.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
