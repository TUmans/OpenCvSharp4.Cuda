...
public class CornersDetector : Algorithm
{
    protected CornersDetector(IntPtr smartPtr, IntPtr rawPtr)
            : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_CornersDetector_delete(p)))
    {
    }
        ...
        double minDistance = 0.0, int blockSize = 3, bool useHarrisDetector = false, double harrisK = 0.04)
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createGoodFeaturesToTrackDetector(
                (int)srcType, maxCorners, qualityLevel, minDistance, blockSize,
                useHarrisDetector ? 1 : 0, harrisK, out var smartPtr));

        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CornersDetector_get(smartPtr, out var rawPtr));
        ...
        OpenCvSharp.Cuda.CudaInputArray? mask = null, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CornersDetector_detect(
    ...
    public int MaxCorners
    {
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_CornersDetector_setMaxCorners(RawPtr, value));
            GC.KeepAlive(this);
        }
    }
    ...
    public double MinDistance
    {
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_CornersDetector_setMinDistance(RawPtr, value));
            GC.KeepAlive(this);
        }
    }
