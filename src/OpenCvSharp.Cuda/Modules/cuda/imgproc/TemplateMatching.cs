...
namespace OpenCvSharp.Cuda
{
    ...
    public class TemplateMatching : Algorithm
    {
        protected TemplateMatching(IntPtr smartPtr, IntPtr rawPtr)
            : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_TemplateMatching_delete(p)))
        {
        }
            ...
            MatType srcType, TemplateMatchModes method, Size? userBlockSize = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_createTemplateMatching((int)srcType, (int)method, blockSize, out var smartPtr));

            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_TemplateMatching_get(smartPtr, out var rawPtr));
            ...
            OpenCvSharp.Cuda.OutputArray result, OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_TemplateMatching_match(RawPtr, image.CvPtr, templ.CvPtr, result.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
