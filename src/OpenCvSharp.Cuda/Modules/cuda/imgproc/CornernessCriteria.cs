...
public class CornernessCriteria : Algorithm
{
    protected CornernessCriteria(IntPtr smartPtr, IntPtr rawPtr)
        : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_CornernessCriteria_delete(p)))
    {
    }
        ...
        MatType srcType, int blockSize, int ksize, double k, BorderTypes borderType = BorderTypes.Reflect101)
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createHarrisCorner((int)srcType, blockSize, ksize, k, (int)borderType, out var smartPtr));

        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CornernessCriteria_get(smartPtr, out var rawPtr));
    ...
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createMinEigenValCorner((int)srcType, blockSize, ksize, (int)borderType, out var smartPtr));
    ...
    public virtual void Compute(OpenCvSharp.Cuda.InputArray src, OpenCvSharp.Cuda.OutputArray dst, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CornernessCriteria_compute(RawPtr, src.CvPtr, dst.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
