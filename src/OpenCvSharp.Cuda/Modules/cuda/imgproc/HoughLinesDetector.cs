...
namespace OpenCvSharp.Cuda
{
    ...
    public class HoughLinesDetector : Algorithm
    {
        protected HoughLinesDetector(IntPtr smartPtr, IntPtr rawPtr)
            : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_delete(p)))
        {
        }
            ...
            float rho, float theta, int threshold, bool doSort = false, int maxLines = 4096)
        {
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_createHoughLinesDetector(
                    rho, theta, threshold, doSort ? 1 : 0, maxLines, out var smartPtr));

            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_HoughLinesDetector_get(smartPtr, out var rawPtr));
            ...
            OpenCvSharp.Cuda.InputArray src, OpenCvSharp.Cuda.OutputArray lines, OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_HoughLinesDetector_detect(RawPtr, src.CvPtr, lines.CvPtr,stream?.CvPtr ?? IntPtr.Zero));
            ...
            OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_HoughLinesDetector_downloadResults(
        ...
        public bool DoSort
        {
            ...
            { 
                ThrowIfDisposed(); 
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_getDoSort(RawPtr, out int val)); 
                GC.KeepAlive(this);
                return val != 0; 
            }
            set 
            { 
                ThrowIfDisposed();
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_setDoSort(RawPtr, value ? 1 : 0)); 
                GC.KeepAlive(this); 
            }
        }
        ...
        public int MaxLines
        {
            get
            {
                ThrowIfDisposed(); NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_getMaxLines(RawPtr, out int val));
                GC.KeepAlive(this);
                return val;
            }
            set 
            { 
                ThrowIfDisposed(); NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_setMaxLines(RawPtr, value));
                GC.KeepAlive(this);
            }
        }
        ...
        public float Rho
        {
            ...
            { 
                ThrowIfDisposed(); 
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_getRho(RawPtr, out float val)); 
                GC.KeepAlive(this); 
                return val; 
            }
            set 
            { 
                ThrowIfDisposed();
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_setRho(RawPtr, value)); 
                GC.KeepAlive(this);
            }
        }
        ...
        public float Theta
        {
            ...
            { 
                ThrowIfDisposed(); 
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_getTheta(RawPtr, out float val)); 
                GC.KeepAlive(this); 
                return val; 
            }
            set 
            { 
                ThrowIfDisposed();
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_setTheta(RawPtr, value)); 
                GC.KeepAlive(this); 
            }
        }
        ...
        public int Threshold
        {
            ...
            { 
                ThrowIfDisposed(); 
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_getThreshold(RawPtr, out int val));
                GC.KeepAlive(this); 
                return val; 
            }
            set 
            {
                ThrowIfDisposed(); 
                NativeMethods.HandleException(NativeMethods_cuda.cuda_HoughLinesDetector_setThreshold(RawPtr, value));
                GC.KeepAlive(this); 
            }
        }
