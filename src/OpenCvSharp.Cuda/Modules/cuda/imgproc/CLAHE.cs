...
public class CLAHE : Algorithm
{
    protected CLAHE(IntPtr smartPtr, IntPtr rawPtr)
        : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_CLAHE_delete(p)))
    {
    }
    ...
    public static CLAHE Create(double clipLimit = 40.0, Size? tileGridSize = null)
    {
        ...
        NativeMethods.HandleException(NativeMethods_cuda.cuda_createCLAHE(
            clipLimit, grid, out var smartPtr));

        NativeMethods.HandleException(NativeMethods_cuda.cuda_CLAHE_get(smartPtr, out IntPtr rawPtr));
    ...
    public virtual void Apply(OpenCvSharp.Cuda.CudaInputArray src, OpenCvSharp.Cuda.CudaOutputArray dst, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_CLAHE_apply(RawPtr, src.CvPtr, dst.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
