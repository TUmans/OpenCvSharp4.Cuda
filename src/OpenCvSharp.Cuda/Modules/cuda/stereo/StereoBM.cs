...
public class StereoBM : OpenCvSharp.Cuda.StereoMatcher
{
    protected StereoBM(IntPtr smartPtr, IntPtr rawPtr)
        : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_StereoBM_delete(p)))
    {
    }
    ...
    public static StereoBM Create(int numDisparities = 64, int blockSize = 19)
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createStereoBM(numDisparities, blockSize, out var smartPtr));

        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_StereoBM_get(smartPtr, out var rawPtr));
        ...
        OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_StereoBM_compute(
