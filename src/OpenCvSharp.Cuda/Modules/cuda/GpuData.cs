    ...
    public sealed class GpuData : DisposableGpuObject
    {
        ...
        public GpuData(ulong size)
        {
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuData_new(size, out var ptr));
        ...
        private void InitSafeHandle(IntPtr p, bool ownsHandle = true)
        {
            SetSafeHandle(new OpenCvCudaPtrSafeHandle(p, ownsHandle,
                static h => NativeMethods_cuda.cuda_GpuData_delete(h)));
        }
        ...
        public IntPtr Data
        {
            ...
            {
                ThrowIfDisposed();
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuData_data(ptr, out var ret));
                GC.KeepAlive(this);
                return ret;
            }
        }
        ...
        public ulong Size
        {
            get
            {
                ThrowIfDisposed();
                NativeMethods.HandleException(
                    NativeMethods_cuda.cuda_GpuData_size(ptr, out var ret));
                GC.KeepAlive(this);
                return (ulong)ret;
            }
        }
