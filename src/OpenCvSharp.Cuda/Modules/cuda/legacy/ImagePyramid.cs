...
namespace OpenCvSharp.Cuda
{
    ...
    public class ImagePyramid : Algorithm
    {
        protected ImagePyramid(IntPtr smartPtr, IntPtr rawPtr)
            : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_ImagePyramid_delete(p)))
        {
        }
            ...
            OpenCvSharp.Cuda.InputArray img, int nLayers = -1, OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_createImagePyramid(img.CvPtr, nLayers, stream?.CvPtr?? IntPtr.Zero, out var smartPtr));
