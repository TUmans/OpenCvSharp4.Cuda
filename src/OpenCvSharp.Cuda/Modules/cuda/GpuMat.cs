...
namespace OpenCvSharp.Cuda
{
    ...
    public class GpuMat : CvObject
    {
        ...
        {
            ThrowIfNotAvailable();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new1(out IntPtr p));
            if (p == IntPtr.Zero)
        ...
        public GpuMat(int rows, int cols, MatType type)
        {
            ...
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new2(rows, cols, (int)type, out IntPtr p));
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        public GpuMat(int rows, int cols, MatType type, IntPtr data, long step)
        {
            ...
            if (cols <= 0)
                throw new ArgumentOutOfRangeException(nameof(cols));
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new3(rows, cols, (int)type, data, (ulong)step, out IntPtr p));
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        {
            ThrowIfNotAvailable();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new6(size, (int)type, out IntPtr p));
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        {
            ThrowIfNotAvailable();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new7(size, (int)type, data, (ulong)step, out IntPtr p));
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        public GpuMat(Mat m)
        {
            ...
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new4(m.CvPtr, out IntPtr p));
            GC.KeepAlive(m);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        ...
        public GpuMat(GpuMat m)
        {
            ...
            if (m is null)
                throw new ArgumentNullException(nameof(m));
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new5(m.CvPtr, out IntPtr p));
            GC.KeepAlive(m);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        ...
        public GpuMat(int rows, int cols, MatType type, Scalar s)
        {
            ...
            if (cols <= 0)
                throw new ArgumentOutOfRangeException(nameof(cols));
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new8(rows, cols, (int)type, s, out IntPtr p));
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        {
            ThrowIfNotAvailable();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new11(size, (int)type, s, out IntPtr p));
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        }
        ...
        public GpuMat(GpuMat m, Range rowRange, Range colRange)
        {
            ...
            if (m is null)
                throw new ArgumentNullException(nameof(m));
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new9(m.CvPtr, rowRange, colRange , out IntPtr p));
            GC.KeepAlive(m);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        ...
        public GpuMat(GpuMat m, Rect roi)
        {
            ...
            if (m is null)
                throw new ArgumentNullException(nameof(m));
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_new10(m.CvPtr, roi, out IntPtr p));
            GC.KeepAlive(m);
            if (p == IntPtr.Zero)
                throw new OpenCvSharpException();
            InitSafeHandle(p);
        ...
        private void InitSafeHandle(IntPtr p, bool ownsHandle = true)
        {
            SetSafeHandle(new OpenCvCudaPtrSafeHandle(p, ownsHandle,
                static h => NativeMethods_cuda.cuda_GpuMat_delete(h)));
        }
        ...
        public static explicit operator GpuMat(Mat mat)
        {
            ...
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_opToGpuMat(mat.CvPtr, out IntPtr ret));
            GC.KeepAlive(mat);
        ...
        public static implicit operator Mat(GpuMat gpumat)
        {
            ...
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_opToMat(gpumat.CvPtr, out IntPtr ret));
            GC.KeepAlive(gpumat);
        ...
        public int Flags
        {
            get
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_flags(CvPtr, out int ret));
                return ret;
            }
        }
        ...
        public int Rows
        {
            get 
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_rows(CvPtr, out int ret));
                return ret;
            }
        }
        ...
        public int Cols
        {
            get
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_cols(CvPtr, out int ret));
                return ret;
            }
        }
        ...
        public unsafe IntPtr Data
        {
            get
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_data(CvPtr, out IntPtr ret));
                return ret;
            }
        }
        ...
        public IntPtr RefCount
        {
            get
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_refcount(CvPtr, out IntPtr ret));
                return ret;
            }
        }
        ...
        public IntPtr DataStart
        {
            get
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_datastart(CvPtr, out IntPtr ret));
                return ret;
            }
        }
        ...
        public IntPtr DataEnd
        {
            get
            {
                ThrowIfDisposed();
                GC.KeepAlive(this);
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_dataend(CvPtr, out IntPtr ret));
                return ret;
            }
        }
        ...
        public IntPtr CudaPtr
        {
            get
            {
                ...
                NativeMethods.HandleException(
                    NativeMethods_cuda.cuda_GpuMat_cudaPtr(CvPtr, out IntPtr ptr));
        ...
        public void UpdateContinuityFlag()
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_updateContinuityFlag(CvPtr));
            GC.KeepAlive(this);
        }
        ...
        {
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_defaultAllocator(out IntPtr ptr));
            return ptr;
        }
        ...
        {
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_getStdAllocator(out IntPtr ptr));
            return ptr;
        }
        ...
        public static void SetDefaultAllocator(IntPtr allocator)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_setDefaultAllocator(allocator));
        }
        #endregion
        #region Indexers
        ...
        public virtual GpuMat this[Rect roi]
        {
            get
            {
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_opRange1(CvPtr, roi, out IntPtr ret));
                GC.KeepAlive(this);
                return new GpuMat(ret);
            }
        }
        ...
        public virtual GpuMat this[Range rowRange, Range colRange]
        {
            get
            {
                NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_opRange2(CvPtr, rowRange, colRange, out IntPtr ret));
                GC.KeepAlive(this);
                return new GpuMat(ret);
            }
        }
        ...
        #region Col/ColRange
                ...
                {
                    ThrowIfDisposed();
                    NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_colRange(CvPtr, startcol, endcol, out IntPtr ret));
                    GC.KeepAlive(this);
                    return new GpuMat(ret);
                }
                ...
                public class ColIndexer : GpuMatRowColIndexer
                {
                    ...
                    public override GpuMat this[int x]
                    {
                        get
                        {
                            ...
                            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_col(parent.ptr, x, out IntPtr matPtr));
                            GC.KeepAlive(this);
                    ...
                    public override GpuMat this[int startCol, int endCol]
                    {
                        ...
                        {
                            parent.ThrowIfDisposed();
                            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_colRange(parent.ptr, startCol, endCol, out IntPtr matPtr));
                            GC.KeepAlive(this);
                            return new GpuMat(matPtr);
                        }
                        set
                        {
                            ...
                            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_colRange(parent.ptr, startCol, endCol, out IntPtr colMatPtr));
                            GC.KeepAlive(this);
        ...
        #region Row/RowRange
                ...
                {
                    ThrowIfDisposed();
                    NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_rowRange(CvPtr, startrow, endrow, out IntPtr ret));
                    GC.KeepAlive(this);
                    return new GpuMat(ret);
                }
                ...
                public class RowIndexer : GpuMatRowColIndexer
                {
                    ...
                    public override GpuMat this[int x]
                    {
                        ...
                        {
                            parent.ThrowIfDisposed();
                            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_row(parent.ptr, x, out IntPtr matPtr));
                            GC.KeepAlive(this);
                            return new GpuMat(matPtr);
                        }
                        set
                    ...
                    public override GpuMat this[int startCol, int endCol]
                    {
                        ...
                        {
                            parent.ThrowIfDisposed();
                            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_rowRange(parent.ptr, startCol, endCol, out IntPtr matPtr));
                            GC.KeepAlive(this);
                            return new GpuMat(matPtr);
                        }
                        set
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_isContinuous(CvPtr, out int res));
            GC.KeepAlive(this);
            return res !=0;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_channels(CvPtr, out int res));
            GC.KeepAlive(this);
            return res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_depth(CvPtr, out int res));
            GC.KeepAlive(this);
            return res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_elemSize(CvPtr, out ulong res));
            GC.KeepAlive(this);
            return (long)res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_elemSize1(CvPtr, out ulong res));
            GC.KeepAlive(this);
            return (long)res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_size(CvPtr, out OpenCvSharp.Size res));
            GC.KeepAlive(this);
            return res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_step(CvPtr, out ulong res));
            GC.KeepAlive(this);
            return (long)res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_step1(CvPtr, out ulong res));
            GC.KeepAlive(this);
            return (long)res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_type(CvPtr, out int res));
            GC.KeepAlive(this);
            return res;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_empty(CvPtr, out int res));
            GC.KeepAlive(this);
            return res != 0;
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods_cuda.cuda_GpuMat_upload(CvPtr, m.CvPtr);
            GC.KeepAlive(this);
            GC.KeepAlive(m);
        }
        ...
        public void Upload(OpenCvSharp.InputArray m, Stream stream)
        {
            ...
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_upload_stream(CvPtr, m.CvPtr, stream.CvPtr));
            GC.KeepAlive(this);
            GC.KeepAlive(m);
            GC.KeepAlive(stream);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods_cuda.cuda_GpuMat_download(CvPtr, m.CvPtr);
            GC.KeepAlive(this);
            GC.KeepAlive(m);
        }
        /// <summary>
        ...
        public void Download(OpenCvSharp.OutputArray m, Stream stream)
        {
            ...
            if (stream == null) throw new ArgumentNullException(nameof(stream));
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_download_stream(CvPtr, m.CvPtr, stream.CvPtr));
            GC.KeepAlive(this);
            GC.KeepAlive(m);
            GC.KeepAlive(stream);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_clone(CvPtr, out IntPtr ret));
            GC.KeepAlive(this);
            return new GpuMat(ret);
        }
        ...
        public void CopyTo(GpuMat dst)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_copyTo1(CvPtr, dst.CvPtr,  IntPtr.Zero));
        ...
        public void CopyTo(GpuMat dst, GpuMat mask)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_copyTo_mask1(CvPtr, dst.CvPtr, mask.CvPtr, IntPtr.Zero));
        ...
        public void CopyTo(GpuMat dst, GpuMat mask, OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_copyTo_mask1(CvPtr, dst.CvPtr, mask.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
        ...
        public void CopyTo(OpenCvSharp.Cuda.OutputArray dst)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_copyTo2(CvPtr, dst.CvPtr, IntPtr.Zero));
        ...
        public void CopyTo(OpenCvSharp.Cuda.OutputArray dst, OpenCvSharp.Cuda.InputArray mask)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_copyTo_mask2(CvPtr, dst.CvPtr, mask.CvPtr, IntPtr.Zero));
        ...
        public void CopyTo(OpenCvSharp.Cuda.OutputArray dst, OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_copyTo2(CvPtr, dst.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
        ...
        public void ConvertTo(GpuMat dst, MatType rtype)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo1(CvPtr, dst.CvPtr, rtype.Value, 1.0, 0.0, IntPtr.Zero));
        ...
        public void ConvertTo(GpuMat dst, MatType rtype, double alpha, double beta, Stream stream)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo1(CvPtr, dst.CvPtr, rtype.Value, alpha, beta, stream?.CvPtr ?? IntPtr.Zero));
        ...
        public void ConvertTo(GpuMat dst, MatType rtype, Stream stream)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo1(CvPtr, dst.CvPtr, rtype.Value, 1.0, 0.0, stream?.CvPtr ?? IntPtr.Zero));
        ...
        public void ConvertTo(OpenCvSharp.Cuda.OutputArray dst, MatType rtype)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo2(CvPtr, dst.CvPtr, rtype.Value, 1.0, 0.0, IntPtr.Zero));
        ...
        public void ConvertTo(OpenCvSharp.Cuda.OutputArray dst, MatType rtype, double alpha, double beta, Stream? stream = null)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo2(CvPtr, dst.CvPtr, rtype.Value, alpha, beta, stream?.CvPtr ?? IntPtr.Zero));
        ...
        public void ConvertTo(OpenCvSharp.Cuda.OutputArray dst, MatType rtype, double alpha, double beta = 0.0)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo2(CvPtr, dst.CvPtr, rtype.Value, alpha, beta,  IntPtr.Zero));
        ...
        public void ConvertTo(OpenCvSharp.Cuda.OutputArray dst, MatType rtype, double alpha, Stream stream)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo2(CvPtr, dst.CvPtr, rtype.Value, alpha, 0.0, stream?.CvPtr ?? IntPtr.Zero));
        ...
        public void ConvertTo(OutputArray dst, MatType rtype, Stream stream)
        {
            ...
            if (dst == null)
                throw new ArgumentNullException(nameof(dst));
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_GpuMat_convertTo2(CvPtr, dst.CvPtr, rtype.Value, 1.0, 0.0, stream?.CvPtr ?? IntPtr.Zero));
        ...
        {
            ThrowIfDisposed();
            NativeMethods_cuda.cuda_GpuMat_assignTo(CvPtr, m.CvPtr, -1);
            GC.KeepAlive(this);
            GC.KeepAlive(m);
        }
        /// <summary>
        ...
        public void AssignTo(GpuMat m, MatType type)
        {
            ...
            if (m is null)
                throw new ArgumentNullException(nameof(m));
            NativeMethods_cuda.cuda_GpuMat_assignTo(CvPtr, m.CvPtr, (int)type);
            GC.KeepAlive(this);
            GC.KeepAlive(m);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_setTo(CvPtr, s, IntPtr.Zero, out IntPtr ret));
            GC.KeepAlive(this);
            return new GpuMat(ret);
        }
        ...
        public void SetTo(Scalar s, OpenCvSharp.Cuda.InputArray mask, Stream stream)
        {
            ...
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_setTo_stream(CvPtr, s, Cv2Cuda.ToPtr(mask), stream.CvPtr, out _));
            GC.KeepAlive(this);
            GC.KeepAlive(mask);
            GC.KeepAlive(stream);
        }
        ...
        public void SetTo(Scalar s,  Stream stream)
        {
            ...
            ThrowIfDisposed();
            // We use Cv2.ToPtr helper to handle the case where mask might be null
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_setTo_stream(CvPtr, s, IntPtr.Zero, stream.CvPtr, out _));
            GC.KeepAlive(this);
            GC.KeepAlive(stream);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_reshape(CvPtr, cn, rows, out IntPtr ret));
            GC.KeepAlive(this);
            return new GpuMat(ret);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods_cuda.cuda_GpuMat_create1(CvPtr, rows, cols, (int)type);
            GC.KeepAlive(this);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods_cuda.cuda_GpuMat_create2(CvPtr, size, (int)type);
            GC.KeepAlive(this);
        }
        ...
        public void Swap(GpuMat mat)
        {
            ...
            NativeMethods_cuda.cuda_GpuMat_swap(CvPtr, mat.CvPtr);
            GC.KeepAlive(this);
            GC.KeepAlive(mat);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods_cuda.cuda_GpuMat_locateROI(CvPtr, out wholeSize, out ofs);
            GC.KeepAlive(this);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_adjustROI(CvPtr, dtop, dbottom, dleft, dright, out IntPtr ret));
            GC.KeepAlive(this);
            return new GpuMat(ret);
        }
        ...
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(NativeMethods_cuda.cuda_GpuMat_ptr(CvPtr, y, out IntPtr res));
            GC.KeepAlive(this);
            return (byte*)res;
        }
        ...
        {
            ThrowIfDisposed();
            if (Cv2Cuda.GetCudaEnabledDeviceCount() < 1)
