...
namespace OpenCvSharp.Cuda
{
    public sealed class Event : DisposableGpuObject
    {
        public Event()
        {
            NativeMethods.HandleException(NativeMethods_cuda.cuda_Event_new1(out IntPtr p));
            InitSafeHandle(p);
        }
        ...
        public Event(EventCreateFlags flags = EventCreateFlags.Default)
        {
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_Event_new2((int)flags, out IntPtr p));
            InitSafeHandle(p);
        }
        ...
        {
            SetSafeHandle(new OpenCvCudaPtrSafeHandle(p, true,
                h => NativeMethods_cuda.cuda_Event_delete(h)));
        }
        ...
        {
            NativeMethods.HandleException(NativeMethods_cuda.cuda_Event_waitForCompletion(CvPtr));
            GC.KeepAlive(this);
        }

        public bool QueryIfComplete()
        {
            NativeMethods.HandleException(NativeMethods_cuda.cuda_Event_queryIfComplete(CvPtr, out int res));
            GC.KeepAlive(this);
            return res != 0;
        }
        ...
        public static float ElapsedTime(Event start, Event end)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_Event_elapsedTime(start.CvPtr, end.CvPtr, out float time));
        ...
        public IntPtr Handle
        {
            get
            {
                ThrowIfDisposed();
                NativeMethods.HandleException(
                    NativeMethods_cuda.cuda_Event_getEvent(ptr, out var handle));
                GC.KeepAlive(this);
                return handle;
            }
        }
        ...
        public static Event WrapEvent(IntPtr rawHandle)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_Event_wrapEvent(rawHandle, out var ptr));
