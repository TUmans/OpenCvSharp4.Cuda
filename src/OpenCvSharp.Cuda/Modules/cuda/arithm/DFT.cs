...
public class DFT : Algorithm
{
    protected DFT(IntPtr smartPtr, IntPtr rawPtr)
        : base(smartPtr, rawPtr, p=> NativeMethods.HandleException( NativeMethods_cuda.cuda_DFT_delete(p)))
    {
    }
    ...
    public static DFT Create(Size dftSize, DftFlags flags)
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createDFT(dftSize, (int)flags, out var smartPtr));

        NativeMethods.HandleException(NativeMethods_cuda.cuda_DFT_get(smartPtr, out IntPtr rawPtr));
        ...
        OpenCvSharp.Cuda.InputArray src, OpenCvSharp.Cuda.OutputArray dst, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_DFT_compute(RawPtr, src.CvPtr, dst.CvPtr, stream?.CvPtr ?? IntPtr.Zero));
