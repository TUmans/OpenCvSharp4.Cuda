...
namespace OpenCvSharp.Cuda
{
    ...
    public class LookUpTable : Algorithm
    {
        protected LookUpTable(IntPtr smartPtr, IntPtr rawPtr)
            : base(smartPtr, rawPtr, p => NativeMethods.HandleException(NativeMethods_cuda.cuda_LookUpTable_delete(p)))
        {
        }
        ...
        public static LookUpTable Create(OpenCvSharp.InputArray lut)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_createLookUpTable(lut.CvPtr, out var smartPtr));

            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_LookUpTable_get(smartPtr, out var rawPtr));
            ...
            OpenCvSharp.Cuda.CudaInputArray src, OpenCvSharp.Cuda.CudaOutputArray dst, OpenCvSharp.Cuda.Stream? stream = null)
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_LookUpTable_transform(RawPtr, src.CvPtr, dst.CvPtr, stream?.CvPtr?? IntPtr.Zero));
