...
namespace OpenCvSharp.Cuda
{
    ...
    public class BufferPool : DisposableGpuObject
    {
        ...
        protected override void DisposeUnmanaged()
        {
            if (ptr != IntPtr.Zero)
            {
                NativeMethods.HandleException(NativeMethods_cuda.cuda_BufferPool_delete(ptr));
            }
        ...
        public GpuMat GetBuffer(int rows, int cols, MatType type)
        {
            ThrowIfDisposed();
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_BufferPool_getBuffer(ptr, rows, cols, type.Value, out var gpuMatPtr));
        ...
        public IntPtr GetAllocator()
        {
            ...
            NativeMethods.HandleException(
                NativeMethods_cuda.cuda_BufferPool_getAllocator(ptr, out var allocatorPtr));
