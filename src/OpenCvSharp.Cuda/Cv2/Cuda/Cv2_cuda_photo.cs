...
public static partial class Cv2Cuda
{
        ...
        float h, int searchWindow = 21, int blockSize = 7, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_fastNlMeansDenoising(src.CvPtr, dst.CvPtr, h, searchWindow, blockSize, ToPtr(stream)));
        ...
        float hLuminance, float photoRender, int searchWindow = 21, int blockSize = 7, OpenCvSharp.Cuda.Stream? stream = null)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_fastNlMeansDenoisingColored(src.CvPtr, dst.CvPtr, hLuminance, photoRender, searchWindow, blockSize, ToPtr(stream)));
