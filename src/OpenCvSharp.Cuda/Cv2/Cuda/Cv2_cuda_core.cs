...
public static partial class Cv2Cuda
{
    ...
    public static void CreateContinuous(int rows, int cols, MatType type, OpenCvSharp.Cuda.CudaOutputArray m)
    {
        ...
        NativeMethods_cuda.cuda_createContinuous1(rows, cols, (int)type, m.CvPtr);
        GC.KeepAlive(m);
    }
    ...
    public static GpuMat CreateGpuMatFromCudaMemory(int rows, int cols, MatType type, IntPtr cudaMemoryAddress, ulong step = 0)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_createGpuMatFromCudaMemory(rows, cols, type.Value, cudaMemoryAddress, (UIntPtr)step, out var ptr));
    ...
    public static bool DeviceSupports(FeatureSet featureSet)
    {
        ...
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_deviceSupports((int)featureSet, out var ret));
    ...
    {
        NativeMethods.HandleException(NativeMethods_cuda.cuda_getCudaEnabledDeviceCount(out int res));
        return res;
    }
    ...
    {
        ThrowIfGpuNotAvailable();
        NativeMethods.HandleException(NativeMethods_cuda.cuda_getDevice(out int res));
        return res;
    }
    ...
    {
        ThrowIfGpuNotAvailable();
        NativeMethods.HandleException(NativeMethods_cuda.cuda_printCudaDeviceInfo(device));
    }
    ...
    {
        ThrowIfGpuNotAvailable();
        NativeMethods.HandleException(NativeMethods_cuda.cuda_printShortCudaDeviceInfo(device));
    }
    ...
    {
        ThrowIfGpuNotAvailable();
        NativeMethods.HandleException(NativeMethods_cuda.cuda_resetDevice());
    }
    #endregion
    ...
    public static void EnsureSizeIsEnough(int rows, int cols, MatType type, OpenCvSharp.Cuda.CudaOutputArray m)
    {
        ...
        if (m is null)
            throw new ArgumentNullException(nameof(m));
        NativeMethods_cuda.cuda_ensureSizeIsEnough(rows, cols, (int)type, m.CvPtr);
        GC.KeepAlive(m);
    }
    ...
    public static void RegisterPageLocked(Mat m)
    {
        ...
        if (m is null)
            throw new ArgumentNullException(nameof(m));
        NativeMethods.HandleException(NativeMethods_cuda.cuda_registerPageLocked(m.CvPtr));
        GC.KeepAlive(m);
    }
    ...
    public static void UnregisterPageLocked(Mat m)
    {
        ...
        if (m is null)
            throw new ArgumentNullException(nameof(m));
        NativeMethods.HandleException(NativeMethods_cuda.cuda_unregisterPageLocked(m.CvPtr));
        GC.KeepAlive(m);
    }
    ...
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_setBufferPoolConfig(deviceId, (UIntPtr)stackSize, stackCount));
    }
    ...
    {
        NativeMethods.HandleException(
            NativeMethods_cuda.cuda_setBufferPoolUsage(on ? 1 : 0));
    }
    #endregion
