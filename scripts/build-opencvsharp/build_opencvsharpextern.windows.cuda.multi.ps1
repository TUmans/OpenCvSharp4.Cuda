...
if ($Build.Count -gt 0) {
    $Targets = $AllTargets | Where-Object { $Build -contains $_.Name }
    $unrecognised = $Build | Where-Object { $AllTargets.Name -notcontains $_ }
    if ($unrecognised) {
        throw "Unrecognised target(s): $($unrecognised -join ', '). Valid values: $($AllTargets.Name -join ', ')"
    }
    Write-Host ">>> Building only: $($Targets.Name -join ', ')" -ForegroundColor Yellow
} else {
    $Targets = $AllTargets
...
foreach ($T in $Targets) {
    ...
    if (-not $OpenCVConfigFile) {
        throw "Failed to find OpenCVConfig.cmake in $InstallDirArch. Has OpenCV been built for $ArchLabel yet?"
    }
