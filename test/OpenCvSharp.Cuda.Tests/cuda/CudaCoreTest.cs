...
public class CudaCoreTest : CudaTestBase
{
    ...
    public void ConvertFp16Test()
    {
        ...
        Cv2Cuda.ConvertFp16(gpuSrc, gpuDst);
        // Assert
    ...
    public void EnsureSizeIsEnough_Test()
    {
        ...
        Cv2Cuda.EnsureSizeIsEnough(100, 100, MatType.CV_8UC1, gpuMat);
        ...
        Cv2Cuda.EnsureSizeIsEnough(200, 200, MatType.CV_8UC1, gpuMat);
    ...
    public void PageLockedMemory_Test()
    {
        ...
        Cv2Cuda.RegisterPageLocked(cpuMat);
        ...
        Cv2Cuda.UnregisterPageLocked(cpuMat);
    ...
    public void SetBufferPool_SetupTest()
    {
        ...
        // Get current device
        int deviceId = Cv2Cuda.GetDevice();

        // Act & Assert
        var exception = Record.Exception(() =>
        {
            // Enable the pool
            Cv2Cuda.SetBufferPoolUsage(true);

            // Configure: 10MB per stack, 5 stacks total
            Cv2Cuda.SetBufferPoolConfig(deviceId, 10 * 1024 * 1024, 5);
        });

        Assert.Null(exception);

        // Clean up: switch back to default (optional)
        Cv2Cuda.SetBufferPoolUsage(false);
    }
    ...
    public void CreateGpuMatFromCudaMemory_Test()
    {
        ...
        using var wrappedGpu = Cv2Cuda.CreateGpuMatFromCudaMemory(
